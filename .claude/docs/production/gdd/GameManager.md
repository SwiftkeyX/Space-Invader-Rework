...
|---|---|---|---|
| Exact handshake for "level cleared → advance": does LevelManager call a GameManager method, or does GameManager subscribe to `LevelManager.OnLevelCleared`? | designer / Claude | Before LevelManager GDD (Tier 2) | **Resolved (2026-06-14)**: `LevelManager.OnEnable` subscribes `OnLevelCleared += GameManager.Instance.HandleLevelCleared`. `HandleLevelCleared(int _)` advances the index or ends the run at level 6. |
